...
const refresh_token = async (req, res, next) => {
  ...
  if (!refresh || !uid) return res.status(401).json({ err: 'credentials unmatched' });
  ...
  try {
    ...
    const user = rows[0];
    if (!user) return res.status(403).json({ err: 'credentials unmatched' });

    const match = await compare(refresh, user.refresh_token);
    if (!match) return res.status(403).json({ err: 'credentials unmatched' });
